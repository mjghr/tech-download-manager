...
func (d *DownloadManager) DownloadQueue(queue *controller.QueueController) {
	logs.Log(fmt.Sprintf("Initializing download queue %s with %d controllers and speed limit %d bytes/s",
		queue.QueueID, len(queue.DownloadControllers), queue.SpeedLimit))

	// Apply queue speed limit to each download controller
	for i, dc := range queue.DownloadControllers {
		if queue.SpeedLimit != 0 {
			logs.Log(fmt.Sprintf("Setting speed limit for download controller %d in queue %s to %d bytes/s from queue",
				i, queue.QueueID, queue.SpeedLimit))
			dc.SpeedLimit = queue.SpeedLimit
		} else {
			logs.Log(fmt.Sprintf("Keeping existing speed limit %d bytes/s for download controller %d in queue %s",
				dc.SpeedLimit, i, queue.QueueID))
		}
	}
	...
	for i := range queue.DownloadControllers {
		...
		go func(dc *controller.DownloadController, idx int) {
			...
			logs.Log(fmt.Sprintf("Starting goroutine for download controller %d: %s", idx, dc.FileName))
			startTime := time.Now()
			err := d.StartDownload(dc)
			if err != nil {
				logs.Log(fmt.Sprintf("Download failed for %s: %v", dc.FileName, err))
				dc.Status = controller.FAILED
			} else {
				dc.Status = controller.COMPLETED
			}
			endTime := time.Now()
			logs.Log(fmt.Sprintf("Download completed for %s | Duration: %v | Status: %v",
				dc.FileName, endTime.Sub(startTime), dc.Status))
		}(queue.DownloadControllers[i], i)
	}
	wg.Wait()

	logs.Log(fmt.Sprintf("All downloads in queue %s completed", queue.QueueID))
}

func (d *DownloadManager) StartDownload(downloadController *controller.DownloadController) error {
	logs.Log(fmt.Sprintf("Preparing to start download for URL: %s", downloadController.Url))
	...
	logs.Log(fmt.Sprintf("Sending HEAD request to determine file details for %s", url))
	response, err := httpRequestSender.SendRequest(reqMethod, url, headers)
	if err != nil {
		logs.Log(fmt.Sprintf("Failed to send HEAD request for %s: %v", url, err))
		return fmt.Errorf("failed to send HEAD request: %w", err)
	}
	defer response.Body.Close()

	contentLength := response.Header.Get("Content-Length")
	logs.Log(fmt.Sprintf("Received Content-Length: %s for %s", contentLength, url))
	contentLengthInBytes, err := strconv.Atoi(contentLength)
	if err != nil || contentLengthInBytes <= 0 {
		logs.Log(fmt.Sprintf("Invalid or empty Content-Length for %s: %v", url, err))
		return fmt.Errorf("invalid Content-Length '%s': %w", contentLength, err)
	}
	logs.Log(fmt.Sprintf("Parsed Content-Length: %d bytes for %s", contentLengthInBytes, url))
	...
	logs.Log(fmt.Sprintf("Calculated %d workers with chunk size %d bytes for %s",
		workers, chunkSize, url))

	fileName, err := util.ExtractFileName(url)
	if err != nil {
		logs.Log(fmt.Sprintf("Failed to extract filename from %s: %v", url, err))
		return fmt.Errorf("failed to extract filename: %w", err)
	}
	logs.Log(fmt.Sprintf("Extracted filename: %s", fileName))
	...
	logs.Log(fmt.Sprintf("Initialized download controller for %s with %d chunks and speed limit %d bytes/s",
		fileName, workers, downloadController.SpeedLimit))

	logs.Log(fmt.Sprintf("Generated byte ranges for %d chunks for %s", len(chunks), fileName))

	tmpPath := util.GiveDefaultTempPath()
	downPath := util.GiveDefaultSavePath()
	logs.Log(fmt.Sprintf("Using temp path: %s and download path: %s", tmpPath, downPath))

	// Create directories if they don't exist
	if err := os.MkdirAll(tmpPath, 0755); err != nil {
		logs.Log(fmt.Sprintf("Failed to create temp directory %s: %v", tmpPath, err))
		return fmt.Errorf("failed to create temp directory %s: %w", tmpPath, err)
	}
	if err := os.MkdirAll(downPath, 0755); err != nil {
		logs.Log(fmt.Sprintf("Failed to create download directory %s: %v", downPath, err))
		return fmt.Errorf("failed to create download directory %s: %w", downPath, err)
	}
	logs.Log(fmt.Sprintf("Successfully ensured directories exist: %s and %s", tmpPath, downPath))
	...
	for idx, byteChunk := range chunks {
		...
		go func(idx int, byteChunk [2]int) {
			defer wg.Done()
			logs.Log(fmt.Sprintf("Starting download of chunk %d for %s (bytes %d-%d)",
				idx, fileName, byteChunk[0], byteChunk[1]))
	...
	logs.Log(fmt.Sprintf("Merging downloads for %s from %s to %s", fileName, tmpPath, downPath))
	err = downloadController.MergeDownloads(tmpPath, downPath)
	if err != nil {
		logs.Log(fmt.Sprintf("Failed to merge downloads for %s: %v", fileName, err))
		return fmt.Errorf("failed to merge downloads: %w", err)
	}

	logs.Log(fmt.Sprintf("Cleaning up temporary files for %s in %s", fileName, tmpPath))
	err = downloadController.CleanupTmpFiles(tmpPath)
	if err != nil {
		logs.Log(fmt.Sprintf("Failed to clean up temporary files for %s: %v", fileName, err))
		return fmt.Errorf("failed to clean up temporary files: %w", err)
	}

	logs.Log(fmt.Sprintf("File successfully generated: %s in %s", fileName, downPath))
	return nil
}
